$(function () {
  ...
  function setFieldError(field, message) {
    const wrapper = field
      .closest(
        ".mb-3, .recipe-filter-group, .meta-grid > div, .full-span, .input-group",
      )
      .first();
  ...
  function validateField(input) {
    ...
    const label =
      field.data("label") ||
      field.closest("div").find("label").first().text() ||
      "This field";
    ...
    if (
      ...
      value &&
      field.attr("min") &&
      Number(value) < Number(field.attr("min"))
    ) {
    ...
    if (type === "file" && value) {
      ...
      const allowed = (
        field.data("allowed-extensions") || allowedImageExtensions.join(",")
      ).split(",");
  ...
  function updateRecipeCardState(data) {
    ...
    if (typeof data.comments_count !== "undefined") {
      ...
      card
        ...
        .text(`${data.comments_count} conversation ${label}`);
    }
    ...
    if (typeof data.saved !== "undefined") {
      ...
      card
        .find(".save-label")
        .text(data.saved ? "Unsave Recipe" : "Save Recipe");
    }
  }
  ...
  function normalizeCommentControls(card, commentItem) {
    ...
    const existingButton = actionSlot.find(
      ".hide-comment-btn, .unhide-comment-btn",
    );
    const isHidden =
      commentItem.data("comment-hidden") === true ||
      commentItem.data("comment-hidden") === "true";
  ...
  function addCommentToCard(data) {
    ...
    if (
      card.find(`.comment-item[data-comment-id="${data.comment_id}"]`).length
    ) {
      return;
    }
  ...
  function removeUnsavedRecipeCard(data) {
    if (
      !window.location.pathname.includes("/saved-recipes") ||
      data.action !== "removed"
    ) {
      return;
    }
  ...
  function removeArchivedRecipeCard(data) {
    ...
    const shouldRemoveFromPage =
      ...
      /\/profile\/\d+$/.test(window.location.pathname);
  ...
  function removeUnarchivedRecipeCard(data) {
    if (
      !window.location.pathname.includes("/archived-recipes") ||
      data.action !== "unarchived"
    ) {
      return;
    }
  ...
  function updateArchiveButton(button, data) {
    ...
    button
      .data("archive-action", nextAction)
      .attr("data-archive-action", nextAction);
  ...
  function updateProfileRecipeStats(data) {
    ...
    if (typeof data.archived_recipes_count !== "undefined") {
      $('[data-stat="archived-recipes-count"]').text(
        data.archived_recipes_count,
      );
    }
  ...
  function addActivityItem(activity, prepend) {
    ...
    if (
      !list.length ||
      !activity.id ||
      list.find(`[data-activity-id="${activity.id}"]`).length
    ) {
      return;
    }
  ...
  function showActivityToast(activity) {
    ...
    const actionText =
      activity.type === "comment" ? "New conversation" : "New like";
    const toast = $(`
  ...
  function loadActivityPage() {
    ...
    $.getJSON(page.data("activity-endpoint"))
      ...
      .fail(function () {
        $("#activity-list").html(
          '<div class="activity-loading">Could not load activity right now.</div>',
        );
      });
  }
  ...
  $(document).on("submit", ".comment-form", function (event) {
    ...
    $.post(`/recipes/${recipeId}/comment`, form.serialize())
      ...
      .fail(function (xhr) {
        const message =
          xhr.responseJSON?.message || "Could not update the conversation.";
        alert(message);
      })
      .always(function () {
  ...
  $(document).on("click", ".archive-btn", function () {
    ...
    const recipeId = button.data("recipe-id");
    const action =
      button.data("archive-action") === "unarchive" ? "unarchive" : "archive";
    ...
    $.post(`/recipes/${recipeId}/${action}`)
      ...
      .fail(function (xhr) {
        const message =
          xhr.responseJSON?.message || "Could not update archive status.";
        alert(message);
      })
      .always(function () {
        button.prop("disabled", false);
  ...
  $(document).on("click", ".hide-comment-btn", function () {
    ...
    $.post(`/comments/${commentId}/hide`)
      ...
      .fail(function (xhr) {
        const message =
          xhr.responseJSON?.message || "Could not hide conversation entry.";
        alert(message);
        button.prop("disabled", false);
      });
  });
  ...
  $(document).on("click", ".unhide-comment-btn", function () {
    ...
    $.post(`/comments/${commentId}/unhide`)
      ...
      .fail(function (xhr) {
        const message =
          xhr.responseJSON?.message || "Could not unhide conversation entry.";
        alert(message);
        button.prop("disabled", false);
      });
  });
